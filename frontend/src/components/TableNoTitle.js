...
const TableNoTitle = React.memo(({ data }) => {
    ...
    const initialEnglishHeaders = {
        ...
        hora: 'Time',
        role: 'Role',
        codigo: 'Code', // Added 'codigo' translation
        tipo: 'Type',   // Added 'tipo' translation
        ubicacion: 'Location', // Added 'ubicacion' translation
        estado: 'Status', // Added 'estado' translation
        // Add any other headers that might appear in your data
    };
    ...
    const filteredData = data.filter(item => {
        ...
        const codeMatch = item.codigo ? item.codigo.toLowerCase().includes(searchText) : false; // Added search for 'codigo'
        const typeMatch = item.tipo ? item.tipo.toLowerCase().includes(searchText) : false;     // Added search for 'tipo'
        const locationMatch = item.ubicacion ? item.ubicacion.toLowerCase().includes(searchText) : false; // Added search for 'ubicacion'
        ...
        return (nameMatch || lastnameMatch || areaMatch || roleMatch || codeMatch || typeMatch || locationMatch) && startMatch && endMatch;
    });
...
const styles = StyleSheet.create({
    ...
    dateButton: {
        marginTop: 5,
        backgroundColor: '#faf9f9', // This style will be overridden by the inline style
    },
    clearButton: {
        marginTop: 5,
    },
...
const mobileStyles = StyleSheet.create({
    ...
    dateButtonMobile: {
        marginTop: 3,
        backgroundColor: '#eee', // This style now applies conditionally
        paddingVertical: 6,
        paddingHorizontal: 8,
    },
    clearButton: {
